...
export const publishPosition = (request: Request, options: Options) =>
  withAuthorPermission(request, {
    action: 'publish',
    entity: 'editorial_board_position',
    execute: async () => {
      // Get the position with author role and reviews
      ...
      if (isNotCoordinator) {
        const hasCoordinatorReview = position.reviews.some(
