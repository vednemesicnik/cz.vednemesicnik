...
export default function RouteComponent({
  ...
  return (
    <AdminPage>
      <AdminHeadline>{position.pluralLabel}</AdminHeadline>
      ...
      <AdminDetailSection title="Základní informace">
        <AdminDetailList>
          <AdminDetailItem label="Klíč">{position.key}</AdminDetailItem>
          <AdminDetailItem label="Označení v množném čísle">
            {position.pluralLabel}
          </AdminDetailItem>
          <AdminDetailItem label="Pořadí">{position.order}</AdminDetailItem>
          <AdminDetailItem label="Stav">
            <AdminStateBadge state={position.state} />
          </AdminDetailItem>
      ...
      <AdminDetailSection title="Schválení">
        <AdminDetailList>
          <Activity mode={position.reviews.length > 0 ? 'visible' : 'hidden'}>
            {position.reviews.map((review) => (
              <AdminDetailItem
                key={review.id}
          ...
          <AdminDetailItem label="Schváleno koordinátorem">
            {position.hasCoordinatorReview ? 'Ano' : 'Ne'}
          </AdminDetailItem>
        </AdminDetailList>
      </AdminDetailSection>
